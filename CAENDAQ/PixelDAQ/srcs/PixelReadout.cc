...
int DAQ::PixelReadout::ReadConfig(){
  ...
  if (defparams.is_open()){
	...
	if(parname == "ExternalTrigger")                {DAQConfig.ExternalTrigger                        = std::stof(parval);++par;}
...
void DAQ::PixelReadout::PrintConfig(){
  ...
  std::cout << std::left << std::setw(big_name) << "## Set External Trigger: " << std::left << std::setw(spacing) << DAQConfig.ExternalTrigger  << "##" << std::endl;
...
int DAQ::PixelReadout::InitialiseTriggers(){
  ...
  std::cout << "------------------Trigger Initialisation---------------------" << std::endl;
  
  //Ala Added this block, please feel free to remove it if you require to do so.˘˘˘
  if(DAQConfig.ExternalTrigger){
    for(int b=0; b<DAQConfig.MAXNB; b++){ 
      //Set the external trigger to get aquire the values only
      if(ret == CAEN_DGTZ_Success){ret = CAEN_DGTZ_SetExtTriggerInputMode(handle[b],CAEN_DGTZ_TRGMODE_ACQ_ONLY);}
    }
  }//Ala Added this block, please feel free to remove it if you require to do so. ^^^^

  else if(DAQConfig.SoftwareTrigger){
   for(int b=0; b<DAQConfig.MAXNB; b++){
	...
	//Get the closest ADC in the read 1 bit = 0.48828125 mV (2000 mV / 2^12bits) 
	float    ADCOffset = Offset_fine*4096/2000;
	
	//Threshold is baseline + user input 
	uint32_t Threshold = (uint32_t) (ADCOffset + DAQConfig.GroupTriggerThresholds[board_address][group]); 
        
        std::cout << group << ": trigger ADC: " << DAQConfig.GroupTriggerThresholds[board_address][group] << ", trigger threshold: "<< Threshold << std::endl;

	if(ret == CAEN_DGTZ_Success){ret = CAEN_DGTZ_SetGroupTriggerThreshold(handle[b], group, Threshold);}

	//Enable the groups for triggering. 	
	int masknumber =  MakeTriggerMasks(group);
	if(ret == CAEN_DGTZ_Success){ret = CAEN_DGTZ_SetGroupSelfTrigger(handle[b],(CAEN_DGTZ_TriggerMode_t) DAQConfig.GroupTriggerMasks[board_address][group],masknumber);}
...
int DAQ::PixelReadout::MakeTriggerMasks(int ch){
  std::string binary; 
  for(int group=7; group>-1; --group){
    if(group == ch) binary += "1";
    else            binary += "0";
  }
...
int DAQ::PixelReadout::InitialiseOffsets(){
  ...
  std::cout << "------------------Offset Initialisation---------------------" << std::endl;
  ...
  for(int b=0; b<DAQConfig.MAXNB; b++){    
    ...
    //Set baseline offset + correction manually (hardcoded)
    //###Correction Factor of 172 I think from the offset on the asic ###
    //Want to set baseline at 1000mV --> input 16-bit --> output 12-bit = 2048

    //regiser address for DC offset for each group: 0x1n98
    uint32_t address_0 = 4248; //group 0 
    uint32_t address_1 = 4504; //group 1 
    uint32_t address_2 = 4760; //group 2 
    uint32_t address_3 = 5016; //group 3 
    uint32_t address_4 = 5272; //group 4 
    uint32_t address_5 = 5528; //group 5 
    uint32_t address_6 = 5784; //group 6 
    uint32_t address_7 = 6040; //group 7 

    uint32_t offset_0 = 35184;
    uint32_t offset_1 = 35184;
    uint32_t offset_2 = 34838;
    uint32_t offset_3 = 34946;
    uint32_t offset_4 = 35082;
    uint32_t offset_5 = 34796;
    uint32_t offset_6 = 34845;
    uint32_t offset_7 = 34851;

    ret = CAEN_DGTZ_WriteRegister(handle[b], address_0, offset_0);
    ret = CAEN_DGTZ_WriteRegister(handle[b], address_1, offset_1);
    ret = CAEN_DGTZ_WriteRegister(handle[b], address_2, offset_2);
    ret = CAEN_DGTZ_WriteRegister(handle[b], address_3, offset_3);
    ret = CAEN_DGTZ_WriteRegister(handle[b], address_4, offset_4);
    ret = CAEN_DGTZ_WriteRegister(handle[b], address_5, offset_5);
...
int DAQ::PixelReadout::StartAcquisition(){
  ...
  while(TotalnumEvents < MaxEvents && time_passed < MaxTime && !quit && ret==0){ 
    ...
    int EventsInFile=0; 
    while(EventsInFile < DAQConfig.MaxEventsPerFile && ret==0){  
	...
	uint32_t buffersize = bsize;

	for (int ev=0;ev<numEvents;ev++) {

	  // Get the Infos and pointer to the event
	  if(ret == CAEN_DGTZ_Success){ret = CAEN_DGTZ_GetEventInfo(handle[b],buffer,bsize,(int32_t ) ev,&eventInfo,&evtptr);}
	  if(ret != CAEN_DGTZ_Success){this->QuitProgram();}
	  ...
	  if(DAQConfig.Verbose){
	    ...
	    std::cout << "Channel Size: " << eventheader.ChSize << std::endl;
	...
	//Free the Event 
	CAEN_DGTZ_FreeEvent(handle[b],(void**) &Evt);
  ...
  if(ret != CAEN_DGTZ_Success) {
    std::cerr << " Errors Data Aquisition here. Error code: " << (int) ret << std::endl;
  }
