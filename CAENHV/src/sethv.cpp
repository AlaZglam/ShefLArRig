...
int monitorchannel(int ch, std::string pol)
{
  if (!init()) return 1;
  ...
  if (!ready)
    {
      num_not_ready++;
      if(pol=="N") printf("\x1b[37;41m V6521N -- CH%i -- %6.1f V -- %6.3f uA -- %s\x1b[39;49m\n",ch,get_voltage_v6521n(ch),get_current_v6521n(ch),stat.c_str());
      if(pol=="P")printf("\x1b[37;41m V6521P -- CH%i -- %6.1f V -- %6.3f uA -- %s\x1b[39;49m\n",ch,get_voltage_v6521p(ch),get_current_v6521p(ch),stat.c_str());

    }
  else
    {
      if(pol=="N")printf("\x1b[32;40m V6521N -- CH%i -- %6.1f V -- %6.3f uA -- %s\x1b[39;49m\n",ch,get_voltage_v6521n(ch),get_current_v6521n(ch),stat.c_str());
      if(pol=="P")printf("\x1b[32;40m V6521P -- CH%i -- %6.1f V -- %6.3f uA -- %s\x1b[39;49m\n",ch,get_voltage_v6521p(ch),get_current_v6521p(ch),stat.c_str());
    }    
...
int main(int argc, char *argv[]) 
{
  ...
  if (!strcmp(argv[i], "--setconfig"))
  ...
  else if(!strcmp(argv[i], "--monitorchannel"))
    {
      int ch = atoi(argv[2]);
      std::string pol = (std::string)argv[3];
      status = monitorchannel(ch,pol);
    }
  ...
  else
    {
      ...
      printf("    --monitorchannel ch pol   Read the parameters from thr VME for a single channel\n");
      status = 1;
    }
